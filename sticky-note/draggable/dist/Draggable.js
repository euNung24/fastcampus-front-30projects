...
function Draggable({
  ...
  return /*#__PURE__*/React.createElement(DraggableContext.Provider, {
    value: {
      position,
      setPosition
    }
...
function Handle({
  ...
  const debouncedDrag = useMemo(() => debounce((e, ...args) => onDrag(e, ...args), 100), [onDrag]);
  const onMouseMove = useCallback(e => {
    const [x, y] = mouseDownPosRef.current;
    const newPosition = [e.clientX - x, e.clientY - y];
    if (newPosition[0] < 0 || newPosition[1] < 0 || newPosition[0] > window.innerWidth || newPosition[1] > window.innerHeight) {
      return;
    }
